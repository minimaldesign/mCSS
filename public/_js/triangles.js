...
export function createTrianglesBackground({ target = ".hero canvas" } = {}) {
  ...
  const canvases =
    typeof target === "string"
      ...
      ? target
      : [target];
  if (!canvases.length || !canvases[0]) return;
  canvases.forEach((canvas) => {
    ...
    for (let y = 0; y <= GRID_ROWS; y++) {
      for (let x = 0; x <= GRID_COLS; x++) {
        ...
        const amp = BASE_AMP + (Math.random() - 0.5) * 3;
        const speed1 = BASE_SPEED1 + (Math.random() - 0.5) * 200;
        const speed2 = BASE_SPEED2 + (Math.random() - 0.5) * 200;
    ...
    function animate(time) {
      for (let p of points) {
        p.z =
          Math.sin(
            time / p.speed1 + p.baseX * 0.04 + p.baseY * 0.06 + p.phase1
          ) *
            p.amp +
          Math.cos(
            time / p.speed2 + p.baseY * 0.07 - p.baseX * 0.03 + p.phase2
          ) *
            (p.amp * 0.7);
      }
