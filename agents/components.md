...
- **`_Playground.jsx`** is the generic, config-driven playground island for component docs pages (template string with `{classes}`/`{attrs}`/`{name}` placeholders, grouped control schema). Authoring API is documented on `/components/start`; reuse it instead of building per-component demo islands.
