# mCSS

The library for CSS lovers.

Current version: **1.2.0** ([changelog](CHANGELOG.md)). Copy it, own it: grab [`dist/mcss.css`](dist/mcss.css) (plus [`dist/mcss.components.css`](dist/mcss.components.css) if you want the component library) or the [source files](src/styles/framework), no package manager involved.

## Learn

- [mCSS methodology](https://mcss.dev/docs/start)
- [Tokens](https://mcss.dev/docs/tokens)
- [Responsive design](https://mcss.dev/docs/media-queries)
- [Grid system](https://mcss.dev/docs/global#grid)
- [Utility classes](https://mcss.dev/docs/helpers)
- [Component library](https://mcss.dev/components/start)

## Connect

- [Discussions](https://github.com/minimaldesign/mCSS/discussions)
- [Twitter](https://x.com/_yannb_)
- [Mailing list](https://mcss.ck.page/900e1d8f88)
- [Email](mailto:hello@mcss.dev)
