...
The default `.wrap_content` has a width of `minmax(200px, 70ch)` by default (see below to customize) and there is a modifier `.wrap_content-fullBleed` that will extend to the full size of the viewport. See [demo](/demos/wrap).
...
[srcAnimation]: https://github.com/minimaldesign/mCSS/blob/main/src/styles/global.animation.css
