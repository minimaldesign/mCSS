...
A [theme][themes] is one `theme.*.css` file of token overrides plus optional style rules, wrapped in its own `@layer theme` block. mCSS imports no theme by default (the default look IS the empty theme layer). You activate at most one from your own [\_global](https://github.com/minimaldesign/mCSS/blob/main/src/styles/_global.css#L13) file and override as much/little as you want from there.
...
It goess inside your `site/` folder, in this order:

- `theme.your-theme.css` for your theme.
