...
- [Interface tokens][interfaceTokens] are an abstraction level to standardize common values across elements and components. For instance, components use `--theme-border-color` instead of the lower level token `--base-200`, so one override restyles every bordered surface.
