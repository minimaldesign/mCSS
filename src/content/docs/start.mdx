...
Atoms are similar to components. The main distinction between components and an atoms is that the atoms don't have a corresponding HTML/Astro/etc. component. For example, the `.button` atom can be added to a `<button>` or a `<a>` HTML element.
