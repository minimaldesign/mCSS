...
- **Source files**: copy [`src/styles/framework/`][framework-src] if you run PostCSS yourself (the source uses `@custom-media`, which needs [postcss-preset-env][presetEnv]).
...
A [theme][themes] is a swappable skin: one `theme.*.css` file of token overrides plus optional style rules, wrapped in its own `@layer theme` block. mCSS imports no theme by default (the default look IS the empty theme layer); you activate at most one from your own entry point, and swapping that import restyles the whole site.
