...
- `global.*.css` for site-wide patterns that are not components: your own opt-in classes (a `.photobook` image treatment, for example) and additions to the mCSS globals.
