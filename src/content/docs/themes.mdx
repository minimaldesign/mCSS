...
@import url(./framework/mcss.components.css); /* optional: the component library */
...
- **Style rules for the rest.** Pseudo-elements, `nth-child` rhythm, and `text-decoration` have no token hooks, so themes may style them directly. Theme files are the one place in mCSS allowed to select component classes from outside (see [`theme.wireframe.css`][srcWireframe] for a real example).
