...
The `Hero` component provides flexible layouts for landing pages, product sites, and marketing. ([Full size demo](/demos/hero).) It supports three variants:
...
The Split layout features an asymmetric split (e.g., `70/30`, `80/20`, etc.) with content on one side and media on the other. The media section fills the available space, cropping as needed. The `ratio` prop defines the grid layout, `contentFirst` controls the DOM order of content and media, and `reversed` visually reverses their order (while the grid ratio remains unchanged).
...
| `height`       | number  | —       | Minimum height in vh (0–100) for the full variant.            |
...
| `contentFirst` | boolean | —       | If set, content appears before media in the DOM (content on the left on desktop). |
| `reversed`     | boolean | —       | Visually reverses the content order set by `contentFirst`.                        |
| `flush`        | boolean | —       | Removes the outer padding from the content section.                               |
...
```
  ...
  <li>
    ...
    >
      ...
      <p>The `contentFirst` prop puts the content first in the DOM (above on mobile and left on desktop).</p>
    </Hero>
