...
The full layout spans the entire viewport with content (headline, sub headline, CTA) centered or aligned over a background image, video, gradient, or animated canvas. Use the `height` prop to control min-height in viewport units.
...
```
  ...
  ratio="62/38"
    ...
    >
      ...
      <p>`contentFirst` is not set, but the content still appears first because of the `reversed` prop. The `flush` prop is used to remove left padding.</p>
    </Hero>
