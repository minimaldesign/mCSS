...
The `.badge` atom is a small label for statuses, categories, and "new" markers: in [cards](/components/card), pricing tiers, nav items, article lists… It is pure CSS; there is no Astro component because a `<span>` with a class is already the whole API.
