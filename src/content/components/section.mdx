...
The `.section` component is the workhorse wrapper marketing pages are made of: a full-width band with vertical rhythm, an optional eyebrow/title/lede header, and background variants. It composes [`.wrap`](/docs/global), so its children align to the same content column as the rest of the page, and a `wide` preset lets grids and galleries escape the reading column.
...
Depends on `.wrap` (`global.wrap.css`), part of the mCSS core.
