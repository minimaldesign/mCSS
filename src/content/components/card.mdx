...
| `--card-*` block in `theme.default.ui.css` | UI tokens (see below)                   | [Github][4] |
...
| `aspectRatio`  | `string`                  | undefined      | Image aspect ratio (available [aspect ratios][ar]); unset, the `--card-aspect-ratio` token applies (golden ratio). |
