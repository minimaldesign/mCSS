...
| `--card-*` block in default theme   | Theme tokens (see below)           | [Github][4] |
...
Badge colors come from the [badge atom](/components/badge)'s `--badge-*` tokens.
...
The `badges` prop takes an array of objects. Each object consists of a `label` property which defines the badge's text, and an optional `color` property that defines the badge's background color. If no `color` is provided, it defaults to `--badge-background-color` from the [badge atom](/components/badge).
