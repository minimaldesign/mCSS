...
The `.header` component is the site-wide top bar: logo, primary navigation, optional actions (theme toggle, CTA button), and a built-in mobile menu behind a hamburger button. The mobile menu is keyboard operable (focus moves into the menu on open and back to the trigger on close, `Escape` closes it) and reflects its state with `aria-expanded`.
...
When using plain HTML you also need the small script that wires the hamburger (toggle `.is-open` and `aria-expanded`, move focus, close on `Escape`); copy it from the bottom of [Header.astro][2].
...
| `currentPath` | `string`    | `Astro.url.pathname`| Used to set `aria-current` (exact match: `page`; same first path segment: `true`). |
