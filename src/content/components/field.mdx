...
The `.field` component composes the bare [form elements](/docs/default-html) into label + control + hint + error message, with validation states driven entirely by CSS.
...
| `--field-*` block in `theme.default.ui.css`  | UI tokens (see table below)           | [Github][4] |
...
[4]: https://github.com/minimaldesign/mCSS/blob/main/src/styles/framework/theme.default.ui.css
...
<Playground
  ...
  <input id="pg-email" name="email" type="email"{attrs} />
  {hint}
  ...
  controls={[
    ...
    { heading: "Content", items: [
      ...
      { type: "text", name: "hint", label: "Hint", default: "We'll never share it.", markup: '<small class="field_hint" id="pg-email-hint">{value}</small>', attr: 'aria-describedby="pg-email-hint"' },
...
<form class="fieldRow" style="margin-bottom: var(--sm3);">
...
Fieldset + the [grid system](/docs/layout#grid) for the two-column row:
