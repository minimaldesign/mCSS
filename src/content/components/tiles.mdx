...
The column count responds to the nearest size container. Every [layout scaffold](/docs/layout#layouts) declares its main column as one, so inside a `.layout` page it just works. Outside a scaffold the wrapper is required: without a container ancestor the queries never match and the list stays single-column.
...
Gaps come from the [grid system](/docs/layout#grid)'s `--grid-column-gap` and `--grid-row-gap`, so tiles and grids share the same rhythm.
