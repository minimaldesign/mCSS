...
The `.footer` component is the site-wide bottom section: link columns on the right, a meta area (copyright, credits, version…) on the left. Columns stack on small screens and spread into a responsive grid from the `--md` breakpoint up. It is pure HTML and CSS, no JavaScript.
...
| `--footer-*` block in default theme | Theme tokens (see table below) | [Github][3] |
