...
The `.featureGrid` component is the classic marketing "three columns of icon + title + text" pattern. It is a documented composition over the mCSS [grid system](/docs/global#grid): one item per `<li>`, one column on mobile, two from `--md`, and 2/3/4 (your pick) from `--lg`.
