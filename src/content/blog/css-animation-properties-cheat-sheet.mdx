...
The only property that requires a specific order is `animation-duration`. It has to be before `animation-delay` (which is optional) since both properties are durations. All other values are unique and can be parsed independently of their order.
