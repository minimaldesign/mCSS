...
- **Grid items:** add, remove, reorder, resize, and click to select (to set `justify-self` / `align-self`)
...
You can change CSS properties such as `justify-items`, `align-items`, etc. **Grid-item properties are only available when a grid item is selected.**
